...
OpenCL::OpenCL(bool useGpu, int platformId, int deviceId)
{
...
void OpenCL::Initialize(const sf::Vector2u& imgSize)
{
	m_dataSize = imgSize;
	m_sizeOfImage = imgSize.x * imgSize.y * 4;
	createKernel(imgSize);
}


void OpenCL::ApplyMargolusRules(sf::Uint8* pixelptr, const sf::Vector2u& imgSize)
{
	...
	event.wait();
	handle_clerror(m_queue.enqueueCopyBuffer(m_bufferTempData, m_bufferData, 0, 0, m_sizeOfImage, NULL, &event));
	event.wait();

	handle_clerror(m_queue.enqueueReadBuffer(m_bufferTempData, CL_TRUE, 0, m_sizeOfImage, pixelptr));
}

void OpenCL::createKernel(const sf::Vector2u& imgSize)
{
	m_globalRange = cl::NDRange(imgSize.y);
	m_localRange = cl::NDRange(static_cast<int>(imgSize.y / m_deviceMaxWorkGroupSize + 1));

	m_bufferData = cl::Buffer(m_context, CL_MEM_READ_ONLY, m_sizeOfImage);
	m_bufferTempData = cl::Buffer(m_context, CL_MEM_WRITE_ONLY, m_sizeOfImage);
	...
	handle_clerror(m_kernelSimpleGeneration.setArg(1, m_bufferDimensionX));
	handle_clerror(m_kernelSimpleGeneration.setArg(2, m_bufferDimensionY));
	handle_clerror(m_kernelSimpleGeneration.setArg(3, m_bufferTempData));
}

std::vector<cl::Platform> OpenCL::getPlatforms() const
{
...
void OpenCL::getDevice(bool useGpu, int platformId, int deviceId)
{
...
void OpenCL::createContext(const cl::Device& device)
{
...
void OpenCL::createProgram(const cl::Context& context, const std::string& sourceCodeFile)
{
	std::ifstream sourceFile(sourceCodeFile);
	if (!sourceFile)
	{
		std::cerr << "OpenCL Error: kernel source file " << sourceCodeFile << " not found!" << std::endl;
...
void OpenCL::createCommandQueue(const cl::Context& context, const cl::Device& device)
{
	int err;
...
cl::Device OpenCL::getDevice(const std::vector<cl::Platform>& platforms, int platformId, int deviceId)
{
...
cl::Device OpenCL::getDevice(const std::vector<cl::Platform>& platforms, int cl_device_type)
{
...
void OpenCL::debugDeviceOutput(const cl::Device& device)
{
	int err;
...
cl::Kernel OpenCL::createKernel(const std::string& functionName) const
{
	int err;
...
std::string OpenCL::cl_errorstring(cl_int err)
{
	switch (err) {
...
void OpenCL::handle_clerror(cl_int err)
{
	if (err != CL_SUCCESS) {
